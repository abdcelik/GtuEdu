...
int String_remove(String* s, int index)
{
	if(s == NULL)
		return 0;
...
int String_removeAllOccurences(String* s, char removedItem)
{
	if(s == NULL)
		return 0;

	for(int i = s->size - 1 ; i >= 0; --i)
		if(s->arr[i] == removedItem)
			String_remove(s, i);
...
int StringArray_remove(StringArray* arr, int index)
{
	if(arr == NULL)
		return -1;
...
int StringArray_removeItems(StringArray* arr, const char* item)
{
	if(arr == NULL || item == NULL)
		return -1;

	for(int i = arr->size - 1 ; i >= 0 ; --i)
		if(String_cmp(arr->arr[i]->arr, item) == 0)
			StringArray_remove(arr, i);
