...
(defun lexer (str)
	(let ((str-lst (str-to-list str)) (token-lst))
		(map nil #'(lambda (token) (setq token-lst (concatenate 'list token-lst (list (tokenize token (get-tokens)))))) str-lst)
		(reverse (pairlis token-lst str-lst))))
